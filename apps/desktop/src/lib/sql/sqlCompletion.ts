...
// Apache Doris (2.1 – 4.x) speaks the MySQL dialect plus its own table-model,
// catalog and index DDL. `MATCH_*` are inverted-index predicates (infix, not
// callable) and `USING INVERTED | NGRAM_BF | ANN` are the index kinds — ANN is
// the Doris 4.0 vector index.
const DORIS_SQL_KEYWORDS = [
  ...MYSQL_SQL_KEYWORDS,
  "AGGREGATE KEY",
  "UNIQUE KEY",
  "DISTRIBUTED BY",
...
// Apache Doris: the MySQL function surface plus the OLAP aggregate-state, array /
// VARIANT and IP helpers, and the Doris 4.0 additions — `SEARCH` (full-text query
// DSL), the approximate vector distances served by ANN indexes, and `AI_*`.
const DORIS_FUNCTION_SIGNATURES = new Map<string, string[]>([
  ...MYSQL_FUNCTION_SIGNATURES,
  ["BITMAP_UNION", ["bitmap"]],
  ["BITMAP_UNION_COUNT", ["bitmap"]],
  ["BITMAP_COUNT", ["bitmap"]],
  ["BITMAP_CONTAINS", ["bitmap", "value"]],
  ["TO_BITMAP", ["expression"]],
  ["HLL_UNION_AGG", ["hll"]],
  ["HLL_CARDINALITY", ["hll"]],
  ["HLL_HASH", ["expression"]],
  ["PERCENTILE_APPROX", ["expression", "percentile"]],
  ["RETENTION", ["...conditions"]],
  ["WINDOW_FUNNEL", ["window", "mode", "timestamp", "...conditions"]],
  ["SEQUENCE_MATCH", ["pattern", "timestamp", "...conditions"]],
  ...
  ["DATE_TRUNC", ["datetime", "unit"]],
  ["DAYS_ADD", ["date", "days"]],
  ["DAYS_DIFF", ["end", "start"]],
  ["TO_DATE", ["datetime"]],
  ["FROM_UNIXTIME", ["unix_timestamp", "format"]],
  ["UNIX_TIMESTAMP", ["datetime"]],
  ["IS_IPV4_STRING", ["string"]],
  ["IS_IPV6_STRING", ["string"]],
  ["TO_IPV4", ["string"]],
  ["TO_IPV6", ["string"]],
  ["IPV4_STRING_TO_NUM", ["string"]],
  ["IPV4_NUM_TO_STRING", ["number"]],
  ["SEARCH", ["dsl"]],
  ["MULTI_MATCH", ["fields", "type", "query"]],
  ["TOKENIZE", ["text", "properties"]],
  ["L2_DISTANCE", ["vector_a", "vector_b"]],
  ["L2_DISTANCE_APPROXIMATE", ["vector_a", "vector_b"]],
  ["INNER_PRODUCT", ["vector_a", "vector_b"]],
  ["INNER_PRODUCT_APPROXIMATE", ["vector_a", "vector_b"]],
